...
with tab_forecast:
    ...
    if not df.empty:
        ...
        for v in list_vendor_fc:
            ...
            if bulan_pilihan == 6:
                # Jika pilih Juni: hitung yang Date In bulan 6 ATAU (Date In <= 5 DAN statusnya FULL)
                mask_supplied = (d_v['BULAN_IN'] == 6) | ((d_v['BULAN_IN'] > 0) & (d_v['BULAN_IN'] <= 5) & (d_v['STATUS'].astype(str).str.upper() == 'FULL'))
            else:
                # Untuk Juli dan seterusnya: normal sesuai bulan yang dipilih
        ...
        try:
            ...
            # Fallback jika pandas server versi lama
